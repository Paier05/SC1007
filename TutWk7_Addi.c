...
BSTNode *searchNode(BSTNode *node, int j);
///////////////////////////// main() /////////////////////////////////////////////
...
int main()
{
	...
	while (c != 0)
	{
		...
		case 2:
			...
			holder = searchNode(&root, j); // You need to code this function
...
BSTNode *searchNode(BSTNode *node, int j)
{
    ...
    if(j > holder->item){
        holder = searchNode(holder->right, j);
    }else{
        holder = searchNode(holder->left, j);
    }
